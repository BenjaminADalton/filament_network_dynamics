...
//
// Simulate molecular motor driven flow in a pinning field channel.
...
int main()
{
    ...
    int N_FILS    = 1;    // initial number of MTs
    ...
    double cryst_length = 6.0e-6;  // length of pins
    ...
    double k_n = 3.5;     // bulk nucleation rate
    ...
    for(i = N_PIN; i < N_FILS; i++)
    {
        ...
        while(nuc_block == 0)
        {
            nuc_block = 1; loop_int = loop_int + 1;
            
            for (unsigned int n = 0; n < N_DIMS; n++)
            {
                mt_new.r_com[n] = (box_vec[n]/xz_nuc_frac)*dis(generator); mt_new.u_vec[n] = 0.0;
            }
            
            mt_new.r_com[1] = (box_vec[1]/y_nuc_frac)*dis(generator);
    ...
    for(i=0;i<T_STEP;i++)
    {
        ...
        if(i % sample_inc_kn == 0)
        {
            if(dis(generator) <= sample_inc_kn*k_n*dt)
            {
                ...
                while(nuc_block == 0)
                {
                    ...
                    // shift by 5.0e-6 to nucleate throughout pinning region
                    mt_new.r_com[1] = (box_vec[1] + 3.0e-6)*dis(generator) - 3.0e-6;
        ...
        // When a filament is removed, the final element in the mt[] vector is
        // placed in its location on the vector, then the final element is removed from
        // vector with the pop_back function...
        if (i % t_depoly == 0)
        {
