...
//
// Simulate two colliding branching filament networks, initated from two
// antiparallel MT seeds. Networks will grow and eventually interact via
...
#define BC_TYPE 3           // select boundary conditions (0,1,2 or 3 = see list below)
#define CL 1                // cross-linkers/motors (0 = off, 1 = on)
#define BRANCH_TYPE 1       // include branching nucleation (0 = off, 1 = on)
...
int main()
{
    ...
    int N_FILS    = 2;    // initial number of MTs
    int N_CLS     = 1500; // total number of available CLs
    int N_BRANCH  = 250;  // total number of available nucleators
    
    // velocity for the motor protein (0.0e-6 for passive)
    double motor_vel1 = 0.15e-6; // kinesin - must be either positive or zero
    
    // for the filament inital state setup
    double init_length = 2.5e-6; // initial filament length
    double nuc_fil_min = 0.5e-6; // newly nucleated filament length
    ...
    double k_n = 0.0;   // bulk nucleation rate
    double k_c = 0.038; // catastrophe rate
    
    // Branching rates
    double k_b_pul     = 2500;   // branching rate per unit length of MT (0.1um^-1s^-1)
    double k_b_detach  = 1.0e-5;  // Make small for no-detatchment (currently not implemented)
    double k_b_react   = 1.0e5;   // Make large for immediate re-activation (currently not implemented)
    
    int force_stable = 1; // 0 = all filaments, 1=split nucleator population, half for each network
    
    // set domain of nucleation
    double br_nuc_upper_y = 6.0e-6;
    double br_nuc_lower_y = -6.0e-6;
    ...
    double k_0_3D = 250; // (m^-1*s^-1*motor^-1 = 2.5e-4um^-1*s^-1*motor^-1)
    double k_off  = 0.033;
    ...
    // avoid walls at nucleation
    double xz_nuc_frac = 1.1;
    double y_nuc_frac  = 1;

    // (0) uy = 1, (1) random, (2) uy = -1, (3) 3D random,  (4) XWall random : note Cryst uy=1
    int rand_or_switch = 6; // for branching - 2 or 6 (in nuc zone, at nuc edges - centrosomal)
    double rand_orient = 0.0;
        
    // print frequency to standard out and txt files
    ...
    const unsigned int sample_inc_branch    = 5000;
    const unsigned int sample_inc_br_detach = 5000;
    const unsigned int sample_inc_br_react  = 5000;
    ...
    const double eta     = 0.02;             // solvent viscosity
    ...
    // nucleator/branch connection quantities
    double delta_eps      = 3.5e-7;
    double k_spr_bra      = 3.0e-5;
    double d_bra[3]       = {0.0};
    double d_bra_0_vec[3] = {0.0};
    double d_bra_0        = 8.0e-8;
    double d_bra_mag      = 0.0;
    double br_delta       = k_B*T/3.0e-12; // k_BT/br_delta = 3pN
    double br_scale       = br_delta*k_spr_bra/(k_B*T);
    ...
    double sqrt2 = sqrt(2.0), sqrt2pi = sqrt(2.0*pi), rand1 = 0.0, k_on_dim = 0.0, f_pi = 0.0, f_pj = 0.0;
    
    int branch_id = 0, loop_int = 0, nuc_block = 0, nuc_index = 0, branch_fil = 0;
    double rand_bra_2 = 0.0, length_acc = 0.0, branch_eps = 0.0, branch_loc = 0.0;
    ...
    vector<branching> branch(N_BRANCH);
    ...
    int nbranch   = 0, n_br_free = 0, cl_nst1 = 0, cl_nst2 = 0;
    ...
    // Initialize the set of initial filaments. In general, there can be more
    // than 2 to begin with. Create initial configuration
    int N_accum = 0;
    ...
    for(i = 0; i < N_FILS; i++)
    {
        ...
        while(nuc_block == 0)
        {
            ...
            mt_new.r_com[0] = (box_vec[0]/xz_nuc_frac)*(dis(generator) - 0.5) + box_vec[0]/2.0;
            mt_new.r_com[1] = (box_vec[1]/y_nuc_frac)*(dis(generator) - 0.5) + box_vec[1]/2.0;
            mt_new.r_com[2] = (box_vec[2]/xz_nuc_frac)*(dis(generator) - 0.5) + box_vec[2]/2.0;
            ...
            else if(rand_or_switch == 6){rand_orient = dis(generator) - 0.5;}
    ...
    // ********************** create antiparallel colliding seeds *************
    // synthetic: for two aligned inital antiparallel seeds, overwrite positions
    // and orientations:
    mt[0].r_com[0] = L_x/2.0; mt[0].r_com[1] = -3.0e-6; mt[0].r_com[2] = L_z/2.0;
    mt[1].r_com[0] = L_x/2.0; mt[1].r_com[1] = 3.0e-6; mt[1].r_com[2]  = L_z/2.0;
    
    mt[0].u_vec[0] = 0.0; mt[0].u_vec[1] = 1.0;  mt[0].u_vec[2] = 0.0;
    mt[1].u_vec[0] = 0.0; mt[1].u_vec[1] = -1.0; mt[1].u_vec[2] = 0.0;
    ...
    ofstream branch_trajectory;
    branch_trajectory.open ("trajectory_branch.txt");
    ...
    for(i=0;i<T_STEP;i++)
    {
        ...
        // bulk nucleation of new filament (not branching nucleationas)
        if(i % sample_inc_kn == 0)
        {
            if(dis(generator) <= sample_inc_kn*k_n*dt)
            {
                ...
                while(nuc_block == 0)
                {
                    ...
                    for (unsigned int n = 0; n < N_DIMS; n++)
                    {
                        mt_new.r_com[n] = (box_vec[n]/xz_nuc_frac)*(dis(generator)-0.5) + box_vec[n]/2.0;
                    }

                    mt_new.r_com[1] = 2.0*box_vec[1]*(dis(generator) - 0.5);
                    ...
                    // controsomal nucleation
                    if(rand_or_switch == 6)
                    {
                        rand_orient = dis(generator) - 0.5;
                        if(rand_orient <= 0.0)
                        {
                                mt_new.r_com[1] = -br_nuc_upper_y;
                                mt_new.u_vec[1] = 1;
                        }
                        else if(rand_orient > 0.0)
                        {
                                mt_new.r_com[1] = br_nuc_upper_y;
                                mt_new.u_vec[1] = -1;
                        }
        ...
        // branching nucleation of new filament
        if(BRANCH_TYPE == 1)
        {
            if(i % sample_inc_branch == 0 && nbranch < N_BRANCH)
            {
                tot_fil_length = 0.0;
                
                for(j = 0; j < N_FILS; j++)
                {
                    tot_fil_length = tot_fil_length + (mt[j].fil_length - delta_eps);
                }
                
                k_on_dim = k_b_pul*tot_fil_length*dt*sample_inc_branch;
                
                // assume that each network has half of the total mass:
                if(force_stable == 1){k_on_dim = k_on_dim/2.0;}
                
                for (l=0; l < N_BRANCH; l++)
                {
                    if(branch[l].branch_state == 0)
                    {
                        if(dis(generator) <= k_on_dim)
                        {
                            filament mt_new;
                            mt_new.filament_init(N_DIMS);
                            mt_new.fil_length = nuc_fil_min;
                            ...
                            loop_int = 0; nuc_block = 0; nuc_index = 0;
                            
                            while(nuc_block == 0)
                            {
                                rand_bra_2 = dis(generator);

                                branch_loc = tot_fil_length*rand_bra_2;
                                ...
                                for(j= 0; j < N_FILS; j++)
                                {
                                    length_acc = length_acc + (mt[j].fil_length - delta_eps);
                                    
                                    if(branch_loc < length_acc)
                                    {
                                        branch_fil = j;
                                        branch_eps = mt[j].fil_length - delta_eps - (length_acc - branch_loc) - mt[j].fil_length/2.0;
                                        
                                        break;
                                    }
                                }
                                
                                nuc_index = nuc_index + 1; nuc_block = 1;
                                
                                // Calculate the plane perpendicular to the mother filament
                                gram_schmidt_3d_2(mt[branch_fil].u_vec, u_perp_3D_1, u_perp_3D_2, e1_3d, e2_3d);
                                
                                double theta_branch = 2*pi*dis(generator);
                                
                                branch_3D[0] = u_perp_3D_1[0]*cos(theta_branch) + u_perp_3D_2[0]*sin(theta_branch);
                                branch_3D[1] = u_perp_3D_1[1]*cos(theta_branch) + u_perp_3D_2[1]*sin(theta_branch);
                                branch_3D[2] = u_perp_3D_1[2]*cos(theta_branch) + u_perp_3D_2[2]*sin(theta_branch);
                                
                                mt_new.u_vec[0] = mt[branch_fil].u_vec[0];
                                mt_new.u_vec[1] = mt[branch_fil].u_vec[1];
                                mt_new.u_vec[2] = mt[branch_fil].u_vec[2];
                                
                                mt_new.r_com[0] = mt[branch_fil].r_com[0] + branch_eps*mt[branch_fil].u_vec[0] + 0.5*mt_new.fil_length*mt[branch_fil].u_vec[0] + d_bra_0*branch_3D[0];
                                mt_new.r_com[1] = mt[branch_fil].r_com[1] + branch_eps*mt[branch_fil].u_vec[1] + 0.5*mt_new.fil_length*mt[branch_fil].u_vec[1] + d_bra_0*branch_3D[1];
                                mt_new.r_com[2] = mt[branch_fil].r_com[2] + branch_eps*mt[branch_fil].u_vec[2] + 0.5*mt_new.fil_length*mt[branch_fil].u_vec[2] + d_bra_0*branch_3D[2];
                                ...
                                for(unsigned int k = 0; k < N_FILS; k++)
                                {
                                    if(k != branch_fil)
                                    {
                                        for(n=0; n<N_DIMS; n++)
                                        {
                                            ri_image[n] = mt[k].r_com[n]; rj_image[n] = mt_new.r_com[n];
                                        }
                                        ...
                                        if(pair.w_ij_mag_single <= 1.05*sig_shift_min){nuc_block = 0; break;}
                                    }
                                }
                                
                                // New daughter filament can not intercept a wall
                                if(BC_TYPE == 2 || BC_TYPE == 3) // herepbc_wall
                                {
                                    ri_s3d[0]  = 0.0;
                                    ri_s3d[1]  = mt_new.r_com[1];
                                    ri_s3d[2]  = mt_new.r_com[2];
                                    
                                    // must normalise:
                                    uwall_3d[0] = 0.0;
                                    uwall_3d[1] = mt_new.u_vec[1];
                                    uwall_3d[2] = mt_new.u_vec[2];
                                    ...
                                    wall_calc.wall_calc_class(mt_new.r_com, ri_s3d, mt_new.u_vec, uwall_3d, mt_new.fil_length, L_wall_3D, N_DIMS,0);
                                    
                                    if(wall_calc.w_ij_mag_out <= sig_min_wall || wall_calc.w_ij_mag_out == 0.0  || mt_new.r_com[0] <= 0.0)
                                    {
                                        nuc_block = 0; break;
                                    }
                                    ...
                                    if(wall_calc.w_ij_mag_out <= sig_min_wall || wall_calc.w_ij_mag_out == 0.0  || mt_new.r_com[0] >= L_x)
                                    {
                                        nuc_block = 0; break;
                                    }
                                    
                                    ri_s3d[0]  = mt_new.r_com[0]; ri_s3d[1]  = mt_new.r_com[1]; ri_s3d[2]  = 0.0;
                                    
                                    // must normalise:
                                    uwall_3d[0] = mt_new.u_vec[0];
                                    uwall_3d[1] = mt_new.u_vec[1];
                                    uwall_3d[2] = 0.0;
                                    umod        = sqrt(uwall_3d[0]*uwall_3d[0] + uwall_3d[1]*uwall_3d[1]);
                                    uwall_3d[0] = uwall_3d[0]/umod;
                                    ...
                                    if(wall_calc.w_ij_mag_out <= sig_min_wall || wall_calc.w_ij_mag_out == 0.0 || mt_new.r_com[2] <= 0.0)
                                    {
                                        nuc_block = 0; break;
                                    }
                                    ...
                                    if(wall_calc.w_ij_mag_out <= sig_min_wall || wall_calc.w_ij_mag_out == 0.0 || mt_new.r_com[2] >= L_z)
                                    {
                                        nuc_block = 0; break;
                                    }
                                }

                                if(nuc_index > 500){ cout  << "WARNING: can not find branch location, to dense" << endl;}
                                if(nuc_index > 2000){ cout << "ABORT: " << endl; abort();}
                                
                                // Set bounds on the branching nucleation space - localised nucletion
                                if(mt_new.r_com[1] < br_nuc_lower_y || mt_new.r_com[1] > br_nuc_upper_y ){nuc_block = 0;}
                                
                                // only accept in the case of population splitting
                                if(force_stable == 1)
                                {
                                    if(mt_new.u_vec[1] >= 0 && l < N_BRANCH/2){nuc_block = 0;}
                                    if(mt_new.u_vec[1] < 0 && l >= N_BRANCH/2){nuc_block = 0;}
                                }
                                
                                branch_id = branch_fil;
                            }
                            
                            if(nuc_block == 1)
                            {
                                // daughter filament accepted - add to MT vector and give properties
                                filament_id_count   = filament_id_count + 1;
                                mt_new.fil_id       = filament_id_count;
                                mt_new.fil_time_nuc = i;
                                ...
                                branch[l].fil_m  = branch_fil;
                                branch[l].fil_d  = new_mt;
                                branch[l].eps_m1 = branch_eps;
                                branch[l].eps_m2 = branch_eps + delta_eps;
                                branch[l].eps_d1 = - mt_new.fil_length/2.0;
                                branch[l].eps_d2 = - mt_new.fil_length/2.0 + delta_eps;
                                
                                branch[l].branch_state = 1;
                                
                                nbranch = nbranch + 1;
                                
                                // update the pair lists, accounting for PBC if necessary
                                for(unsigned int k = 0; k < N_FILS-1; k++)
                                {
                                    for(n=0; n<N_DIMS; n++)
                                    {
                                    ...
                                    if(BC_TYPE == 1)
                                    {
                                        for(n=0;n<N_DIMS;n++)
                                        {
                                            if(mt[new_mt].r_com[n] - mt[k].r_com[n] > box_vec[n]/2.0){ri_image[n] = mt[new_mt].r_com[n]-box_vec[n];}
                                            else if(mt[new_mt].r_com[n] - mt[k].r_com[n] < -box_vec[n]/2.0){ri_image[n] = mt[new_mt].r_com[n] + box_vec[n];}
                                        }
                                    }
                                    if(BC_TYPE == 2) // herepbc_wall
                                    {
                                        if(mt[new_mt].r_com[1] - mt[k].r_com[1] > box_vec[1]/2.0){ri_image[1] = mt[new_mt].r_com[1]-box_vec[1];}
                                        else if(mt[new_mt].r_com[1] - mt[k].r_com[1] < -box_vec[1]/2.0){ri_image[1] = mt[new_mt].r_com[1] + box_vec[1];}
                                    }
            ...
            // Randomly detatch branching connection points according to rate k_b_detach
            // will also deactive the nucleator for some time. Reactivate below
            // if(i % sample_inc_br_detach == 0)
            // {
            //     for(j=0; j < N_BRANCH; j++)
            //     {
            //         if(branch[j].branch_state == 2)
            //         {
            //             if(dis(generator) <= k_b_detach*dt*sample_inc_br_detach*exp(branch[j].extension_mag*br_scale))
            //             {
            //                 branch[j].branch_deactivate(); nbranch = nbranch - 1;
            //             }
            //         }
            //     }
            // }
            
            // Reactivate nucleator when necessary
            // if(i % sample_inc_br_react == 0)
            // {
            //     for(j=0; j < N_BRANCH; j++)
            //     {
            //         if(branch[j].branch_state == 2)
            //         {
            //             if(dis(generator) <= k_b_react*dt*sample_inc_br_detach)
            //             {
            //                 branch[j].branch_state = 0;
            //             }
            //         }
            //     }
            // }
            
            nbranch = 0; for (j=0; j < N_BRANCH; j++){if(branch[j].branch_state == 1){nbranch = nbranch + 1;}}
        ...
        if (i % t_depoly == 0)
        {
            for (n=0; n < N_FILS; n++)
            {
                if(mt[n].growth_phase == 2)
                {
                    ...
                    for(j=0; j < N_BRANCH; j++)
                    {
                        if(branch[j].branch_state == 1)
                        {
                            if(branch[j].fil_m == n || branch[j].fil_d == n )
                            {
                                branch[j].branch_deactivate();
                    ...
                    else if(n < N_FILS - 1)
                    {
                        ...
                        for(j=0; j < N_BRANCH; j++)
                        {
                            if(branch[j].branch_state == 1)
                            {
                                if(branch[j].fil_m ==  N_FILS-1)
                                {
                                    branch[j].fil_m =  n;
                                }
                                if(branch[j].fil_d ==  N_FILS-1)
                                {
                                    branch[j].fil_d = n;
                                }
                            }
        ...
        if(CL == 1)
        {
            ...
            // along the total length of all filaments.
        ...
        // branch connection-point transformations due to changing MT length
        for (j=0; j < N_BRANCH; j++)
        {
            if(branch[j].branch_state == 1)
            {
                branch[j].branch_walk(mt[branch[j].fil_m].fil_length,mt[branch[j].fil_d].fil_length, dt,mt[branch[j].fil_m].growth_phase,mt[branch[j].fil_d].growth_phase, v_g, v_s);
            }
        }
        for (j=0; j < N_BRANCH; j++)
        {
            if(branch[j].branch_state == 1)
            {
                if(branch[j].eps_m2 >= 0.5*mt[branch[j].fil_m].fil_length)
                {
                    branch[j].branch_deactivate();
                    nbranch = nbranch - 1;
                }
                if(branch[j].eps_d2 >= 0.5*mt[branch[j].fil_d].fil_length)
                {
                    branch[j].branch_deactivate();
                    nbranch = nbranch - 1;
                }
                if(mt[branch[j].fil_m].growth_phase == 2 || mt[branch[j].fil_d].growth_phase == 2){branch[j].branch_deactivate(); nbranch = nbranch - 1;}
            }
        }
        ...
        for (j=0; j < cl_nst2; j++)
        {
            ...
            {
                // only detatch connection j, and shuffle properties if need
                cl[cl_st2_list[j]].state = 1;
                cl[cl_st2_list[j]].nlist_clean();
                cl[cl_st2_list[j]].epsilon_i = cl[cl_st2_list[j]].epsilon_j;
                cl[cl_st2_list[j]].fil_i = cl[cl_st2_list[j]].fil_j;
        ...
        for (j=0; j < N_FILS; j++){for(n=0; n<N_DIMS; n++){mt[j].r_eqnm[n] = 0.0;} mt[j].u_eqnm[0] = 0.0; mt[j].u_eqnm[1] = 0.0; mt[j].u_eqnm[2] = 0.0;}
        
        //************ update pair-lists for the appropriate boundary condition **********************
        
        // BC_TYPE 0: No PBC and no Walls - open space
        // BC_TYPE 1: PBC in (x, y, z) for 3D - no walls
        // BC_TYPE 2: Walls in (x, z) with PBC in y
        // BC_TYPE 3: Walls in (x, z) and open BC in y
        ...
        // calculate Eqns of motion due to branching connections - one for each connection
        if(BRANCH_TYPE == 1)
        {
            for(unsigned int k = 0; k < N_BRANCH; k++)
            {
                if(branch[k].branch_state == 1)
                {
                    for(n=0;n<N_DIMS;n++)
                    {
                        ri_image[n] = mt[branch[k].fil_m].r_com[n];
                        rj_image[n] = mt[branch[k].fil_d].r_com[n];
                    }
                    
                    ri_image = pair.image_calc_BC_pair(BC_TYPE, N_DIMS, ri_image, rj_image, box_vec);
                    
                    // first linker for the branch pair
                    
                    d_bra_mag = 0.0;
                    
                    for(n=0; n<N_DIMS; n++)
                    {
                        d_bra[n]  = (ri_image[n] + branch[k].eps_m1*mt[branch[k].fil_m].u_vec[n]) - (rj_image[n] + branch[k].eps_d1*mt[branch[k].fil_d].u_vec[n]);
                        d_bra_mag = d_bra_mag + d_bra[n]*d_bra[n];
                    }
                    
                    d_bra_mag = sqrt(d_bra_mag);
                    
                    branch[k].extension_mag = d_bra_mag - d_0;
                    
                    for(n=0; n<N_DIMS; n++)
                    {
                        d_bra_0_vec[n] = (d_bra_0/d_bra_mag)*d_bra[n];
                    }
                    
                    // update the r_vec equations of motion
                    for(n=0; n<N_DIMS; n++)
                    {
                        mt[branch[k].fil_m].r_eqnm[n] = mt[branch[k].fil_m].r_eqnm[n] - k_spr_bra*(d_bra[n] - d_bra_0_vec[n]);
                        mt[branch[k].fil_d].r_eqnm[n] = mt[branch[k].fil_d].r_eqnm[n] + k_spr_bra*(d_bra[n] - d_bra_0_vec[n]);
                    }
                    
                    u_id_dot = mt[branch[k].fil_m].u_vec[0]*(d_bra[0]-d_bra_0_vec[0]) + mt[branch[k].fil_m].u_vec[1]*(d_bra[1] - d_bra_0_vec[1]) + mt[branch[k].fil_m].u_vec[2]*(d_bra[2] - d_bra_0_vec[2]);
                    u_jd_dot = mt[branch[k].fil_d].u_vec[0]*(d_bra[0]-d_bra_0_vec[0]) + mt[branch[k].fil_d].u_vec[1]*(d_bra[1] - d_bra_0_vec[1]) + mt[branch[k].fil_d].u_vec[2]*(d_bra[2] - d_bra_0_vec[2]);
                    
                    // update the u_vec equations of motion
                    for(n=0;n<N_DIMS;n++)
                    {
                        mt[branch[k].fil_m].u_eqnm[n] = mt[branch[k].fil_m].u_eqnm[n] + k_spr_bra*branch[k].eps_m1*(u_id_dot*mt[branch[k].fil_m].u_vec[n] - (d_bra[n] - d_bra_0_vec[n]));
                        mt[branch[k].fil_d].u_eqnm[n] = mt[branch[k].fil_d].u_eqnm[n] - k_spr_bra*branch[k].eps_d1*(u_jd_dot*mt[branch[k].fil_d].u_vec[n] - (d_bra[n] - d_bra_0_vec[n]));
                    }
                    
                    // second linker for the branch pair
                    
                    d_bra_mag = 0.0;
                    
                    for(n=0; n<N_DIMS; n++)
                    {
                        d_bra[n]  = (ri_image[n] + branch[k].eps_m2*mt[branch[k].fil_m].u_vec[n]) - (rj_image[n] + branch[k].eps_d2*mt[branch[k].fil_d].u_vec[n]);
                        d_bra_mag = d_bra_mag + d_bra[n]*d_bra[n];
                    }
                    ...
                    for(n=0;n<N_DIMS;n++)
                    {
                        mt[branch[k].fil_m].u_eqnm[n] = mt[branch[k].fil_m].u_eqnm[n] + k_spr_bra*branch[k].eps_m2*(u_id_dot*mt[branch[k].fil_m].u_vec[n] - (d_bra[n] - d_bra_0_vec[n]));
                        mt[branch[k].fil_d].u_eqnm[n] = mt[branch[k].fil_d].u_eqnm[n] - k_spr_bra*branch[k].eps_d2*(u_jd_dot*mt[branch[k].fil_d].u_vec[n] - (d_bra[n] - d_bra_0_vec[n]));
                    }
                }
            }
        }
        ...
        if (i % t_write == 0)
        {
            ...
            // print branching nucleator trajectories
            branch_trajectory << i << "\t" << 0 << "\t" << 2*nbranch << "\t";

            for (j=0; j < N_BRANCH; j++)
            {
                if(branch[j].branch_state == 1)
                {
                    branch_trajectory << 2 << "\t" << mt[branch[j].fil_m].r_com[0] + branch[j].eps_m1*mt[branch[j].fil_m].u_vec[0] << "\t" << mt[branch[j].fil_m].r_com[1] + branch[j].eps_m1*mt[branch[j].fil_m].u_vec[1] << "\t" << mt[branch[j].fil_m].r_com[2] + branch[j].eps_m1*mt[branch[j].fil_m].u_vec[2] << "\t" << mt[branch[j].fil_d].r_com[0] + branch[j].eps_d1*mt[branch[j].fil_d].u_vec[0] << "\t" << mt[branch[j].fil_d].r_com[1] + branch[j].eps_d1*mt[branch[j].fil_d].u_vec[1] << "\t" << mt[branch[j].fil_d].r_com[2] + branch[j].eps_d1*mt[branch[j].fil_d].u_vec[2] << "\t";
                    branch_trajectory << 2 << "\t" << mt[branch[j].fil_m].r_com[0] + branch[j].eps_m2*mt[branch[j].fil_m].u_vec[0] << "\t" << mt[branch[j].fil_m].r_com[1] + branch[j].eps_m2*mt[branch[j].fil_m].u_vec[1] << "\t" << mt[branch[j].fil_m].r_com[2] + branch[j].eps_m2*mt[branch[j].fil_m].u_vec[2] << "\t" << mt[branch[j].fil_d].r_com[0] + branch[j].eps_d2*mt[branch[j].fil_d].u_vec[0] << "\t" << mt[branch[j].fil_d].r_com[1] + branch[j].eps_d2*mt[branch[j].fil_d].u_vec[1] << "\t" << mt[branch[j].fil_d].r_com[2] + branch[j].eps_d2*mt[branch[j].fil_d].u_vec[2] << "\t";
                }
            }
            branch_trajectory << "\n";
            
            // Not sure what this is for...
            int N_grow = 0; for(j = 0; j < N_FILS; j++){if(mt[j].growth_phase == 0){N_grow = N_grow + 1;}}
        ...
        if(i >= start_write)
        {
            if (i % t_write_2 == 0)
            {
                ...
                id_trajectory << double(i)*dt << "\t" << N_FILS << "\t" << filament_id_count << "\t";
                id_orientation << double(i)*dt << "\t" << N_FILS << "\t" << filament_id_count << "\t";
                if(N_DIMS == 3)
                {
                    for (j = 0; j < N_FILS; j++)
                    {
        ...
        if (i % t_print == 0)
        {
            ...
            std::cout << "cl bound: " << cl_nst1 + cl_nst2 << ",    state 1: " << cl_nst1 << ",    state 2: " << nt_cl1_1 << ",    nbranch: " << nbranch << ",    total length: " << tot_fil_length << endl;
            cout << endl;
        }
    }
    // end of main loop
